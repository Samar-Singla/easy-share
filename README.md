...
```bash
git clone [https://github.com/iiitiansamar/EasyShare.git]
cd EasyShare
```
...
The signaling server runs on a Cloudflare Worker. You will need to deploy it separately. The code for the signaling server can be found in the `Websocket-Server_EasyShare` directory of this repository.
...
1.  In the root of the `EasyShare` project, create a new file named `.env.local`.
